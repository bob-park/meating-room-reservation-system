...
public class MeetingRoomResponseBodyFields {
    ...
    public static final FieldDescriptor[] MEETING_ROOM_RESPONSE = {
        ...
        fieldWithPath("name").description("회의실 이름").type(JsonFieldType.STRING),
        fieldWithPath("isActive").description("회의실 활성화 여부").type(JsonFieldType.BOOLEAN),
        fieldWithPath("createdDate").description("회의실 생성일").type(JsonFieldType.STRING),
        fieldWithPath("lastModifiedDate").description("회의실 수정일").type(JsonFieldType.STRING)
            .optional(),
